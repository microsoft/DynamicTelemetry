...
it to reduce the cost of our services by locating and dropping a highly verbose log
...
- Use Azure Monitor and KQL language to locate an unexpectedly expensive
  OpenTelemetry log.

- Utilize the Durable ID feature on OpenTelemetry to locate the line of code
...
By using the query below in Application Insights, we can utilize the Durable ID
found in our logs to pinpoint specific log messages or identify any
...
a Durable ID. What was once multiple pages of rows has now been
...
## Utilize the Durable ID to locate the line of code responsible for the expense

It should be immediately apparent that using a Durable ID instead of a
